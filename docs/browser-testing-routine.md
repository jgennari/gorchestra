...
| DRAFT-02 | Edit one session from two tabs; switch sessions and reload each; repeat after send/queue and with older saved-version records | Drafts restore directly into the composer with text/skills together; this tab's local draft wins conflicts; empty local drafts stay cleared after reload; incoming draft edits can fill an empty mounted composer but never replace local text/skills/images or interrupt sending; no recovery list or write-back echo | Regression/release |
...
| SEND-05 | Queue two messages, then Stop with an empty composer; repeat with a draft or while typing during the pending cancellation | Successful Stop restores only the oldest queued text/skills into a still-empty composer; existing text, whitespace, skills, images, and pending image reads are preserved; remaining queue stays queued; no automatic send | Regression/device |
| SEND-06 | Use “Move to composer” beside a queued row's X, with a populated composer; repeat offline, with denied storage, a lost DELETE response, and session navigation during the request | Explicit move replaces text/skills/images, focuses the composer, and removes only the selected row; X remains delete-only; save the copy locally before removal; no send or cancellation; uncertain removal is reported without losing the copy | Regression/device |
...
| PUSH-01 | A subscribed client visibly watches session A when it completes, fails, cancels, or requests permission | One event ACK for that device/session/sequence; no server push to that endpoint and no competing local OS notification | Regression/physical device |
| PUSH-02 | Repeat while watching session B, with the app hidden/locked, and with a second subscribed device | Unviewed events still get one push per device; foreground suppression on one device never mutes the other | Regression/physical device |
| PUSH-03 | Drop/delay the ACK, disconnect SSE, suspend the client, or expire its ACK cache | Normal push delivery remains available; no lingering presence mute or acknowledgement retries; received classic pushes still display through the service worker | Regression/physical device |
...
September 7 notification change: source review identified competing foreground SSE `showNotification` and server-push paths for the same terminal/permission event. Automatic local OS notifications are removed; explicit local notification tests remain. A visible selected session now acknowledges the exact event for its current push subscription. The server waits one second before session push delivery and skips only matching endpoint/session/sequence ACKs. ACKs are memory-only, bounded to 1024 entries, and expire after two minutes; there is no presence heartbeat. Failed or late ACKs leave delivery intact. This is best-effort suppression, not recall of a push already sent. Classic received pushes still display a notification, as required by [WebKit's user-visible push contract](https://webkit.org/blog/12945/meet-web-push/).

Notification validation uses mocked push senders/subscriptions, HTTP route tests, frontend hook tests, and selected-session SSE integration. Do not interpret these as physical iOS banner verification. Run PUSH-01 through PUSH-03 on an authorized device after promotion, using fresh synthetic sessions and no production subscription copied into a test database. Record the actual loaded bundle; an old cached frontend still has its competing local-alert path.

Physical-device follow-up on September 7: after updating the installed iPhone PWA, the user reported no notification for an open-session completion, then confirmed the background completion test behaved correctly. This confirms those completion smoke cases by user report, not the full permission/failure, locked-device, or delayed-network matrix.
